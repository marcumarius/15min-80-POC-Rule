...
static bool RevGatePass(int dir, float px, float vwapv, float minSpace, float maxSpace,
                        const float* walls, int nWalls, bool useVWAP, bool useSpace)
{
    if (dir>0)   // LONG
    {
        if (useVWAP && vwapv>0.0f && !(px>vwapv)) return false;
        if (useSpace){ float nn=1e30f; for(int i=0;i<nWalls;++i){ float w=walls[i]; if(w>0 && w>px && (w-px)<nn) nn=w-px; }
            if(nn<minSpace) return false; if(maxSpace>0 && nn>maxSpace) return false; }   // too close OR no-man's-land
    }
    else if (dir<0)   // SHORT
    {
        if (useVWAP && vwapv>0.0f && !(px<vwapv)) return false;
        if (useSpace){ float nn=1e30f; for(int i=0;i<nWalls;++i){ float w=walls[i]; if(w>0 && w<px && (px-w)<nn) nn=px-w; }
            if(nn<minSpace) return false; if(maxSpace>0 && nn>maxSpace) return false; }
    }
    return true;
}
...
SCSFExport scsf_PriorDayNY_ValueArea(SCStudyInterfaceRef sc)
{
    SCInputRef In_SessionStart=sc.Input[0], In_SessionEnd=sc.Input[1], In_ValueAreaPct=sc.Input[2],
        ...
        In_ProxFlag=sc.Input[96], In_ProxMaxATR=sc.Input[97];
    ...
    if (sc.SetDefaults)
    {
        ...
        In_ProxFlag.Name="Filter: flag no-man's-land signals (far from structure)"; In_ProxFlag.SetYesNo(1);
        In_ProxMaxATR.Name="Filter: max dist to structure (x ATR)"; In_ProxMaxATR.SetFloat(0.5f); In_ProxMaxATR.SetFloatLimits(0.1f,3.0f);
        ...
        if (showHUD && liveDay)
        {
            ...
            if (In_RevEnable.GetYesNo() && curPDidx>=0)
            {
                ...
                for(int i=0;i<3;++i){ int s2; int d=DetectRej(sc,lastClosed,Wr,Lv[i],tolr,mT,mR,cfb,momF,s2);
                    if(d!=0 && !RevGatePass(d,sc.Close[lastClosed],vwapL,minSpaceL,(In_ProxFlag.GetYesNo()&&atrDaily>0?In_ProxMaxATR.GetFloat()*atrDaily:0.0f),wallsL,4,gV,gS)) d=0;
            ...
            // ---- Structure-proximity filter: flag "no-man's-land" signals ----
            // 6mo study: signals >~0.5xATR from the nearest structural level in their
            // direction win far less (21% vs 36%). Flag + downgrade, don't hard-drop.
            bool noMansLand=false;
            if (In_ProxFlag.GetYesNo() && netSign!=0 && atrDaily>0)
            {
                const float px2=sc.Close[lastClosed];
                const float structs[6]={ref.POC,ref.VAH,ref.VAL,(weeklyValid?WVPOC:0.0f),(weeklyValid?PWH:0.0f),(weeklyValid?PWL:0.0f)};
                float nearest=1e30f;
                for(int i=0;i<6;++i){ float L=structs[i]; if(L<=0) continue;
                    float d = (netSign>0)?(L-px2):(px2-L);            // distance to level in trade dir
                    if (d>0 && d<nearest) nearest=d; }
                if (nearest>In_ProxMaxATR.GetFloat()*atrDaily){
                    noMansLand=true;
                    if (convFinal[0]=='H') convFinal="MOD";          // downgrade HIGH->MOD
                    else if (convFinal[0]=='M') convFinal="LOW";
                    SCString a2; a2.Format("%s | NO-MANS-LAND (low prob)", agree.GetChars()); agree=a2;
                }
            }
    ...
    if (In_RevEnable.GetYesNo() && total>=1)
    {
        ...
        for (int b=startB; b<=lastClosed; ++b)
        {
            ...
            for(int i=0;i<3;++i){ int s2; int d=DetectRej(sc,b,Wr,Lv[i],atrTol,mT,mR,cfb,momF,s2);
                if(d!=0 && !RevGatePass(d,sc.Close[b],vwapB,minSpace,(In_ProxFlag.GetYesNo()&&atrDaily>0?In_ProxMaxATR.GetFloat()*atrDaily:0.0f),wallsB,4,gV,gS)) d=0;
